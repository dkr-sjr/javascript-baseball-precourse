...
## 구현할 기능 목록
필요 클래스
1. BaseBallGame - 숫자 야구 게임 웹페이지의 모든 로직을 담당할 클래스

필요 메서드
1. constructor - 변수 생성과 첫 초기화를 위한 생성자
2. initialization - 정답 변수를 MissionUtils 라이브러리를 사용하여 초기화 - 재시작 버튼과 바인드 할 함수
3. play - 확인 버튼과 바인드 할 함수로서 입력된 값을 정답과 비교하고, 결과를 저장할 함수
4. getresult - play 함수를 통해 저당된 결과를 반환하는 함수
5. compare - 정답과 입력값을 비교해줄 함수
6. exception - '예외 함수 alert' 실행

index.html 내부에서 play 함수의 실행 결과에 따른 결과값을 불러와 출력해주기
