...
## 구현할 기능 목록
필요 클래스
1. BaseBallGame - 숫자 야구 게임과 관련된 모든 것을 행할 클래스
2. IOTunnel - 웹에서 입력을 받아오고 웹으로 문자를 출력할 클래스

필요 메서드
BaseBallGame
1. 초기화 함수 - 맞춰야 할 값을 랜덤으로 설정할 함수
2. 비교 결과 출력 함수 - 입력 값과 정답을 비교해서 결과를 반환할 함수
3. 하나 하나 비교 함수 - 인덱스 값을 받아서 결과를 반환할 함수 - 요구 사항을 지키기 위해 분리
4. 입력 체크 함수 - 들어온 입력이 형식에 맞는지 체크하는 함수
5. 예외 함수 - 예외가 발생했다는 것을 알릴 함수
6. 생성자 - 생성하는 동시에 초기화 함수로 랜덤변수 설정

IOTunnel
1. 값 읽어올 함수 - 웹에서 입력된 값을 읽어 올 함수
2. 확인 버튼 함수 - 확인 버튼이 눌린 것을 확인 할 함수
3. 재시작 버튼 함수 - 재시작 버튼이 눌린것을 확인 할 함수
4. 값 출력 함수 - 게임 결과를 웹에 출력 할 함수
